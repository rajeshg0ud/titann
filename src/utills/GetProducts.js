        ...
        {products && products.campaignResponses?.map((pro, i) => (
          ...
          pro.payload && pro.payload.products?.map((item, j) => (
           ...
           <Link to='/Product' key={item.id ? item.id : `product_${j}`}>
            <div className='m-5 mx-5 w-[207px] hover:shadow-lg cursor-pointer justify-self-start  mt-7' onClick={()=>dispatch(addProduct(item))}>
              <img src={item.attributes.imageUrl.value} className='w-[207px] rounded-sm object-cover' alt={item.attributes.name.value} />
              <div className=' p-3'>
                <p className=' font-semibold overflow-hidden line-clamp-2 mb-2'>{item.attributes.name.value}</p>
                ...
                <span className=' text-red-600'>{" "+ Math.ceil((item.attributes.listPrice.value - item.attributes.price.value) / item.attributes.listPrice.value * 100)}% Off</span></p>)
                }
            </div>
            </div>
            </Link>))))
